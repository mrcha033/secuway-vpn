...
$SkillRoot = Join-Path (
    $RepositoryRoot
) 'plugins\secuway-vpn\skills\secuway-vpn'
...
$BundledManifestLines = @('bundled_native_runtime_validated=false')
$BundledEvidencePath = Join-Path $DistRoot 'bundled-runtime-evidence.txt'
if (-not $SkipBundledValidation) {
    ...
    foreach ($BundledAsset in @(
        ...
        if ($ActualHash -ne [string]$Record.sha256) {
            throw (
                'bundled ARM64 asset hash mismatch: ' +
                $BundledAsset.Relative
            )
        }
        $FreshHash = (
            Get-FileHash -LiteralPath $BundledAsset.Fresh -Algorithm SHA256
        ).Hash.ToLowerInvariant()
        if ($FreshHash -ne $ActualHash) {
            throw (
                'fresh ARM64 build differs from bundled asset: ' +
                $BundledAsset.Relative
            )
        }
    }
    & (Join-Path $ExperimentRoot 'test-on-windows.ps1') `
        ...
        -OpenVpnRoot (Join-Path $ExtractRoot 'OpenVPN') |
        Tee-Object -FilePath $BundledEvidencePath
    $BundledManifestLines = @(
        "bundled_provider_sha256=$(
            (
                Get-FileHash -Algorithm SHA256 $BundledProvider
            ).Hash.ToLowerInvariant()
        )"
        "bundled_provider_smoke_sha256=$(
            (
                Get-FileHash -Algorithm SHA256 $BundledSmoke
            ).Hash.ToLowerInvariant()
        )"
...
if (-not $SkipBundledValidation) {
    $BundledEvidenceHash = (
        Get-FileHash -Algorithm SHA256 $BundledEvidencePath
    ).Hash.ToLowerInvariant()
    $ChecksumLines += (
