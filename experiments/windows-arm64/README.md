...
Run `.github/workflows/secuway-windows-arm64-provider.yml` on GitHub's
...
An asset-refresh workflow may pass `-SkipBundledValidation` to
`ci/build-and-test.ps1` while producing a new candidate. That mode still runs
the full fresh-provider native KAT and OpenVPN discovery, but deliberately
does not compare against or execute the checked-in provider it is intended to
replace. Its manifest records `bundled_native_runtime_validated=false`.
