...
`.github/workflows/secuway-windows-portable.yml` runs the same checks on
...
This public repository deliberately does not include a self-hosted tunnel
workflow. Run `Invoke-InteractiveServiceSmoke.ps1` manually on an approved
private host, or invoke it from a private operations repository whose runner,
environment, and enrollment access are separately controlled. Never pass a
profile, password, OTP, or private-key material through public workflow inputs
or secrets, and never upload the profile or OpenVPN log as an artifact.
