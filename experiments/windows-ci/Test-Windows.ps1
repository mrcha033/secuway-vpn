...
$portableRoot = Join-Path $repositoryRoot "portable"
$skillRoot = Join-Path (
    $repositoryRoot
...
if (-not $OutputDirectory) {
    $OutputDirectory = Join-Path $repositoryRoot "build\windows-ci\$nativeArch"
}
...
switch ($Phase) {
    "Portable" {
        ...
        $bundledCli = Join-Path $skillRoot (
            "assets\windows-$nativeArch\secuway.exe"
        )
