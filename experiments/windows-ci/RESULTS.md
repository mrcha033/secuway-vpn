...
## Historical native GitHub Actions evidence

These links are the pre-split `mrcha033/skills` validation baseline. They
preserve provenance for the imported implementation, but they are not evidence
from the standalone repository workflows.
...
The historical portable run executed the Go test suite, DPAPI test, CLI,
status boundary,
...
resolution. The standalone public repository intentionally has no self-hosted
tunnel workflow; live validation must be run manually on an approved private
host or from a private operations repository. Provider loading, doctor output,
and cached enrollment state are not evidence of a working VPN tunnel.
